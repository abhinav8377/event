...
export default function ChatSection({
  ...
  useEffect(() => {
      ...
      try {
        ...
        if (active) {
          dispatch(
            pushToast({
              type: "error",
              message: (e as Error).message || "Cannot open chat",
            }),
          )
          onBack()
        }
    ...
    const joinRoom = () =>
      socket.emit("community:join", communityId, (r: any) => {})
  ...
  const handleLeave = async () => {
    try {
      ...
      dispatch(
        pushToast({ type: "success", message: "You left the community" }),
      )
    } catch {
      // ignore
    }
  ...
  const handleVote = (messageId: string, optionIndex: number) => {
    ...
    socketRef.current.emit(
      "community:poll:vote",
      { communityId, messageId, optionIndex },
      (r: any) => {
        if (!r?.ok) {
          dispatch(
            pushToast({ type: "error", message: r?.error || "Vote failed" }),
          )
        }
      },
    )
    ...
    <>
      <div className="border-b border-border pb-3">
        ...
        <p className="mt-1 flex items-center gap-1 text-xs text-muted-foreground">
          ...
          {chat.community.memberCount} total members
        </p>
      </div>
      <p className="py-2 font-mono text-xs uppercase tracking-wider text-muted-foreground">
        Members
      </p>
          ...
          >
            ...
            <span className="truncate text-sm text-foreground">
              ...
              {typingUsers[m.id] !== undefined && (
                <span className="ml-1.5 text-[10px] italic text-primary">
                  typing...
                </span>
              )}
              {m.isOrganizer && (
    ...
    <div className="flex h-full flex-col overflow-hidden">
        ...
        {showMembers && (
          <div className="fixed inset-0 z-50 lg:hidden">
            ...
            <motion.div
              ...
              className="absolute right-4 top-4 flex max-h-[calc(100%-2rem)] w-80 max-w-[calc(100%-2rem)] flex-col overflow-hidden rounded-2xl border border-border bg-card p-4 shadow-xl"
            >
      ...
      <div className="flex min-h-0 flex-1 flex-col overflow-hidden">
        <div className="grid min-h-0 flex-1 grid-cols-1 gap-4 lg:grid-cols-[280px_1fr]">
          {/* Left panel — desktop only; mobile uses the Info drawer above */}
          <Card className="hidden h-full min-h-0 flex-col p-4 lg:flex">
            {memberPanelBody}
          </Card>
              ...
              {messages.length === 0 ? (
                ...
                messages.map((msg) => {
                  ...
                  return (
                    <div
                      key={msg.id}
                      className="group flex flex-col items-stretch gap-1"
                    >
                      {isPoll ? (
                        <div
                          className={`self-center w-full max-w-md ${mine ? "self-end" : "self-start"}`}
                        >
